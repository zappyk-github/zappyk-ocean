...
execute_command() {
    ...
    [ -n "$rep" ] && cmd="$yum_cmd $yum_opt $yum_exc $yum_rep=$rep $ope"
...
#CZ#execute_command true "$oper"
    for repo in $repositories; do
        execute_command true "$oper" "$repo"
    done

#CZ#execute_command true "$oper" "$repo"
done
